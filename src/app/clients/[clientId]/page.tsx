  ...
  return (
    <div className="project-profile-container">
      ...
      <div className="project-detail-body">
        <div className="open-section" style={{ padding: 'var(--space-6)' }}>
          ...
          <Suspense
            fallback={
              <div className="projects-grid">
                {[1, 2, 3].map((i) => (
                  <div key={i} className="enhanced-project-card skeleton-card">
                    <div className="project-card-header">
                      <div className="skeleton-heading" style={{ width: '60%' }}></div>
                      <div className="skeleton-text short" style={{ marginTop: 6 }}></div>
                    </div>
                  </div>
                ))}
              </div>
  ...
  try {
    ...
    return (
      <div className="client-hero">
        ...
        {/* Row 2: Client details inline */}
        <div className="hero-details-row">
          {client.companyName && (
            <div className="hero-detail">
              <span className="hero-detail-label">Company</span>
              <span className="hero-detail-value">{client.companyName}</span>
            </div>
          )}
          {client.phone && (
            <div className="hero-detail">
              ...
              <span className="hero-detail-value">
                <a href={`tel:${client.phone}`} className="hero-detail-link">{client.phone}</a>
              </span>
            </div>
          )}
          {client.address && (
            <div className="hero-detail">
              ...
              <span className="hero-detail-value">{client.address.split('\n')[0]}</span>
            </div>
          )}
          <div className="hero-detail">
        ...
        <div className="hero-stats-bar">
          <div className="hero-stat">
            <span className="hero-stat-number">{client.projects.filter(p => !p.archived).length}</span>
...
async function ClientProjects({ clientId }: { clientId: string }) {
  try {
    ...
    const renderGrid = (projects: typeof client.projects, emptyMsg: string) =>
      projects.length > 0 ? (
        <div className="projects-grid">
